...
RUN sed -i '/^  datasource:/,/^  data:/ s|^    password:.*|    password: ${RAGENT_DB_PASSWORD}|' bootstrap/src/main/resources/application.yaml \
    ...
    && chmod +x mvnw \
    && ./mvnw -B -ntp -pl bootstrap -am clean package -DskipTests
