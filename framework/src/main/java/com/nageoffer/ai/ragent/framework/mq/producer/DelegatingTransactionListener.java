...
public class DelegatingTransactionListener implements RocketMQLocalTransactionListener {
    ...
    public RocketMQLocalTransactionState checkLocalTransaction(Message message) {
        ...
        try {
            boolean committed = check(checker, message.getPayload());
    ...
    /**
     * Broker 回查投递的是消息字节，需要按 checker 声明的载荷类型反序列化，
     * 不能直接把 payload 强转为 MessageWrapper
     */
    private <T> boolean check(TransactionChecker<T> checker, Object payload) throws IOException {
        JavaType messageType = objectMapper.getTypeFactory()
                .constructParametricType(MessageWrapper.class, checker.bodyType());
        MessageWrapper<T> wrapper = objectMapper.readValue(payloadBytes(payload), messageType);
        return checker.check(wrapper);
    }

    private byte[] payloadBytes(Object payload) throws IOException {
        ...
        throw new IOException("无法识别的回查载荷类型: " + (payload == null ? "null" : payload.getClass().getName()));
    }
}
