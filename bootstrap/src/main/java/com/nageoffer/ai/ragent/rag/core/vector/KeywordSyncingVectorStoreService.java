 ...
 * 一处覆盖全部向量写调用点，无需改动任何调用方
 * <p>
 * 关键词写入为 best-effort：失败只记日志、不回滚向量、不中断主链路，因为关键词检索是非必须能力
 * 若要求万无一失的异构一致，建议改为监听 binlog（CDC）异步写 ES，而非同步双写
 */
@Slf4j
public class KeywordSyncingVectorStoreService implements VectorStoreService {
    ...
    public void indexDocumentChunks(String collectionName, String docId, List<VectorChunk> chunks) {
        ...
        syncKeyword("docId=" + docId, () -> keywordIndexService.indexDocumentChunks(collectionName, docId, chunks));
    }
    ...
    public void updateChunk(String collectionName, String docId, VectorChunk chunk) {
        ...
        syncKeyword("docId=" + docId, () -> keywordIndexService.updateChunk(collectionName, docId, chunk));
    }

    @Override
    public void deleteDocumentVectors(String collectionName, String docId) {
        delegate.deleteDocumentVectors(collectionName, docId);
        syncKeyword("docId=" + docId, () -> keywordIndexService.deleteDocumentIndex(collectionName, docId));
    }

    @Override
    public void deleteChunkById(String collectionName, String chunkId) {
        delegate.deleteChunkById(collectionName, chunkId);
        syncKeyword("chunkId=" + chunkId, () -> keywordIndexService.deleteChunkById(collectionName, chunkId));
    }
    ...
    public void deleteChunksByIds(String collectionName, List<String> chunkIds) {
        ...
        syncKeyword("chunkIds=" + chunkIds, () -> keywordIndexService.deleteChunksByIds(collectionName, chunkIds));
    }

    /**
     * best-effort 执行关键词同步，失败仅告警，不影响向量主链路
     */
    private void syncKeyword(String target, Runnable action) {
        try {
            action.run();
        } catch (Exception e) {
            log.warn("关键词索引同步失败，已跳过 {}", target, e);
        }
    }
}
